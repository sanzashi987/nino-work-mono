...
const getDefaultUrl = (params: IdentifierSource) => {
  const { sourceName, name, version, user, rakToken, isDebugger } = params;
  ...
  return `${baseUrl}${userPath}/mobile/${name}/${version}/static/${sourceName}.json?rak-token=${rakToken}`;
};
const getFileUrl = (params: IdentifierSource, source: Record<string, any>) => {
  const { name, version, user, dashboardId, rakToken, isDebugger } = params;
  if (isDebugger) return getDebugUrl(params);
  return `${canvasApiService}/canvas-pro-mobile/V1/files/file/${
    source.fileId
  }?code=${name}&comVersion=${version}${
    user ? '&cType=1' : ''
  }&screenId=${dashboardId}&rak-token=${rakToken}`;
};
