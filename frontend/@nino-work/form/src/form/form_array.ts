...
> {
  constructor(
    model: ArrayModel<ExtractFormArrayValue<TControl>, any>,
    initialValue: ExtractFormArrayValue<TControl> = []
  ) {
  ...
  override setValue(
    value: TypedOrUntyped<TControl, IsAny<TControl, any[], FormRawValue<TControl>[]>, any>,
  ...
  override patchValue(
    value: TypedOrUntyped<TControl, IsAny<TControl, any[], FormValue<TControl>[]>, any>,
    options?: Object
  ): void {
    if (!Array.isArray(value)) {
      console.warn('expected to receive an array as value');
  ...
  override _deriveValue(): void {
    this.valueReactive.set(this.controls.map(control => control.value));
  }
